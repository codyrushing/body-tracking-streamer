...
class ImageProcessor extends ConfigurableTransform {
  ...
  async convertPoseToBinaryMask(pose){
    const keypointPositions = pose.keypoints.map(k => k.position);
    const xExtent = d3.extent(keypointPositions.map(d => d.x));
    const yExtent = d3.extent(keypointPositions.map(d => d.y));
    console.log(xExtent);
